...
Le type de carte choisi par l’hôte est conservé dans `meta.mapType` (`standard`, `hourglass`, `archipelago` ou `volcano`). Comme la graine, cette valeur est lue avant la construction locale de la carte par chaque client. Le calendrier des éruptions, l’alerte et les impacts choisis sont ensuite transmis dans le `snapshot` autoritaire. Les règles actuelles acceptent ces valeurs sans modification.
